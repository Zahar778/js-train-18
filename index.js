...
function getAge(age) {
  ...
  return { error: error.message, name: error.name }; // Повертаємо об'єкт з ім'ям та текстом помилки
}
...
function getBookById(books, id) {
  try {
    ...
    if (!book) {
      const error = Error(`Книга з ID ${id} не знайдена!`);
...
function decodeURIComponentWrapper(encodedString) {
  try { 
  const decodeURI = decodeURIComponent(encodedString);
  return decodeURI
} catch ( error ) { 
  return { error: error.message, name: error.name };
}
  ...
  function findEvenNumber(numbers) {
    try {
      ...
      if (evenNumber === undefined) {
        throw new Error(`[${numbers}] - У масиві немає чисел, що діляться на 2 без остачі!`);
      }
...
function evaluateExpression(expression) {
  try {
    ...
    if (isNaN(result)) {
      throw new EvalError(`${expression} не є коректним математичним виразом.`);
    }
    return result;
  } catch (error) {
    return error instanceof EvalError ? `${expression} ${error.message}` : error.toString();
  }
